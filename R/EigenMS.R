...
eig_norm1 = function(m, treatment, prot.info, write_to_file=''){
  ...
  retval = list(m=m, treatment=treatment, my.svd=my.svd,
                ...
                complete=complete, toplot1=toplot1, #Tk=Tk,
...
eig_norm2 = function(rv) {
  ...
  bias_m = array(NA, c(nrow(pres), ncol(pres))) # , n.treatment))
  numsamp = dim(pres)[2]
  numpep = dim(pres)[1]
  ...
  colnames(bias_m) = colnames(pres) # these have same number of cols
  rownames(bias_m) = rownames(pres)
  bias_M = data.frame(present, bias_m)
  rownames(bias_M) = rownames(pres)  # rownames(rv$normalized)
  ...
  return(list(normalized=final, norm_m=y_rescaled, eigentrends=V0, rescrange=maxrange,
              norm.svd=toplot3, exPeps=exPeps,bias=bias_M))
